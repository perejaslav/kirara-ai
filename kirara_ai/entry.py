...
_interrupt_count = 0  # счётчик прерываний
...
async def check_update():
    ...
    latest_version, _ = await get_latest_pypi_version("kirara-ru")
    # fallback на имя старого пакета для совместимости
    if latest_version == "0.0.0":
        latest_version, _ = await get_latest_pypi_version("kirara-ai")
    logger.info(f"Текущая версия: {running_version}, последняя версия: {latest_version}")
    backend_update_available = version.parse(latest_version) > version.parse(running_version)
    if backend_update_available:
        logger.warning(f"Доступна новая версия {latest_version}. Рекомендуется обновиться.")
        logger.warning("Скачать обновление можно через веб-панель (Система → Обновление).")
...
def _signal_handler(*args):
    ...
    if _interrupt_count == 1:
        if not shutdown_event.is_set():
            logger.warning("Получен сигнал прерывания. Остановка приложения...")
            shutdown_event.set()
    elif _interrupt_count == 2:
        logger.warning("Сигнал получен повторно. Нажмите Ctrl+C ещё раз для принудительного завершения...")
    else:
        logger.warning("Третий сигнал прерывания — принудительное завершение...")
        os._exit(1)
...
def init_tracing_system(container: DependencyContainer):
    ...
    logger.info("Инициализация системы трассировки...")
    ...
    logger.info("Система трассировки инициализирована")
...
def init_application() -> DependencyContainer:
    ...
    if os.path.exists(config_path):
        ...
        logger.warning(
            f"Файл конфигурации {config_path} не найден — используется конфигурация по умолчанию"
        )
        logger.warning(
            "Скопируйте config.yaml.example в config.yaml (или data/config.yaml) и заполните своими данными"
        )
        config = GlobalConfig()
    ...
    logger.info("Поиск встроенных плагинов...")
    plugin_loader.discover_internal_plugins()
    logger.info("Поиск внешних плагинов...")
...
def run_application(container: DependencyContainer):
    ...
    logger.info("Запуск адаптеров мессенджеров...")
    ...
    signal.signal = lambda *args: None  # type: ignore

    try:
        logger.success("Kirara RU запущена — ожидание сообщений...")
        logger.success(
            f"Веб-панель: http://127.0.0.1:{web_server.listen_port}/"
        )
        ...
        logger.info("Завершение системы памяти...")
        memory_manager.shutdown()

        try:
            tracing_manager = container.resolve(TracingManager)
            logger.info("Завершение системы трассировки...")
            ...
            logger.error(f"Ошибка при завершении трассировки: {e}")
        ...
        try:
            ...
            logger.error(f"Ошибка при остановке адаптеров: {e}")
